<?php
 /**
 * Class for show tables with data
 * The class is a child class of a copy of a renamed WP_List_Table
 ...
 */
class Your_Plugin_BFT_Display_Table extends Your_Plugin_BFT_List_Table{
	/**
	 * Retrieve columns
	 *
	 * @since		1.0.0
	 ...
	 */
	protected $plugin_id = "bft-internationalization";

	/**
	 * Prepare the table for the visualization
	 * 
	 * BFT Mini version
	 * 
	 * @since		1.0.0
	 ...
	 * 
	 */
	public function display_table($display_table_data) {
	}
	...
	/**
	 * Make sortable all original colums
	 * 
	 * BFT Mini version
	 * 
	 * @since		1.0.0
	 ...
	 * Return data of defined and undefined columns
	 * 
	 * Columns can be hidden or sortable
	 * 
	 * BFT Mini version
	 * 
	 * @since		1.0.0
	...
	/**
	 * Activate the checkboxes for the "cb" colum
	 * 
	 * BFT Mini version
	 * 
	 * @since		1.0.0
	 ...
	 */
	//$this->debug_log_write("Your Plugin plugin: function_name_or_string");
...
/**
 * Copy of WP_List_Table of WordPress 5.8.2
 * The name class was changed
 */
 ...
 * @access private
 */
class Your_Plugin_BFT_List_Table {
	 ...
	 *                            Default empty
	 *     @type boolean   $ajax     Whether the list table supports Ajax. This includes loading
	...
	public function __construct( $args = array() ) {
		...
		if ( empty( $this->modes ) ) {
			$this->modes = array(
				'list'    => $this->__( 'Compact view' ),
				'excerpt' => $this->__( 'Extended view' ),
			);
		}
	}
	 ...
	 * @return mixed|boolean Return value of the callback, false otherwise.
	 */
...
<p class="search-box">
	...
	protected function bulk_actions( $which = '' ) {
		...
		echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . $this->__( 'Select bulk action' ) . '</label>';
		echo '<select name="action' . $two . '" id="bulk-action-selector-' . esc_attr( $which ) . "\">\n";
		echo '<option value="-1">' . $this->__( 'Bulk actions' ) . "</option>\n";
		...
		submit_button( $this->__( 'Apply' ), 'action', '', false, array( 'id' => "doaction$two" ) );
		echo "\n";
	}
	 ...
	 * @param boolean     $always_visible Whether the actions should be always visible.
	...
	protected function row_actions( $actions, $always_visible = false ) {
		...
		$out .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . $this->__( 'Show more details' ) . '</span></button>';
	...
	protected function months_dropdown( $post_type ) {
		...
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value="0"><?php $this->_e( 'All dates' ); ?></option>
		<?php
		foreach ( $months as $arc_row ) {
			...
			printf(
				...
				sprintf( $this->__( '%1$s %2$d' ), $wp_locale->get_month( $month ), $year )
			);
		}
		?>
		</select>
	...
	protected function comments_bubble( $post_id, $pending_comments ) {
		...
		$approved_only_phrase = sprintf(
			...
			$this->_n( '%s comment', '%s comments', $approved_comments ),
			$approved_comments_number
		);

		$approved_phrase = sprintf(
			/* translators: %s: Number of comments. */
			$this->_n( '%s approved comment', '%s approved comments', $approved_comments ),
			$approved_comments_number
		);

		$pending_phrase = sprintf(
			/* translators: %s: Number of comments. */
			$this->_n( '%s pending comment', '%s pending comments', $pending_comments ),
		...
		if ( ! $approved_comments && ! $pending_comments ) {
			...
			printf(
				...
				$approved_comments_number,
				$pending_comments ? $this->__( 'No approved comments' ) : $this->__( 'No comments' )
			);
		}
		...
		if ( $pending_comments ) {
			...
			printf(
				...
				$pending_comments_number,
				$approved_comments ? $this->__( 'No pending comments' ) : $this->__( 'No comments' )
			);
		}
	}
	...
	protected function pagination( $which ) {
		...
		$output = '<span class="displaying-num">' . sprintf(
			...
			$this->_n( '%s item', '%s items', $total_items ),
		...
		if ( 'bottom' === $which ) {
			...
			$total_pages_before = '<span class="screen-reader-text">' . $this->__( 'Current Page' ) . '</span><span id="table-paging" class="paging-input"><span class="tablenav-paging-text">';
		} else {
			$html_current_page = sprintf(
				"%s<input class='current-page' id='current-page-selector' type='text' name='paged' value='%s' size='%d' aria-describedby='table-paging' /><span class='tablenav-paging-text'>",
				'<label for="current-page-selector" class="screen-reader-text">' . $this->__( 'Current Page' ) . '</label>',
				$current,
				strlen( $total_pages )
			);
		}
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			static $cb_counter = 1;
			$columns['cb']     = '<label class="screen-reader-text" for="cb-select-all-' . $cb_counter . '">' . $this->__( 'Select All' ) . '</label>'
...
<table class="wp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		return $column_name === $primary ? '<button type="button" class="toggle-row"><span class="screen-reader-text">' . $this->__( 'Show more details' ) . '</span></button>' : '';
	}
	...
	public function ajax_response() {
		...
		if ( isset( $this->_pagination_args['total_items'] ) ) {
			$response['total_items_i18n'] = sprintf(
				...
				$this->_n( '%s item', '%s items', $this->_pagination_args['total_items'] ),
